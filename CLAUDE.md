...
- ElevenLabs: chave `sk_` (51 chars) na env var `ELEVENLABS_API_KEY` do environment; o setup grava em `.env` na raiz do video-use. Escopos necessários: TTS, STT Scribe, sound-generation, voices_read (todos confirmados em 2026-08-18 com probes sem gasto: POST vazio devolve 422 quando o escopo está liberado e 401 quando não está). Plano **free, 10.000 créditos/mês** (conferido 2026-08-18): suficiente só para testes curtos; trilhas e transcrições longas vão estourar a cota rápido. Voz da marca para narração: **PENDENTE (voice_id, modelo e settings)**.
