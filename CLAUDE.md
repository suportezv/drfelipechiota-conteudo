...
| `raw.githubusercontent.com` | **403** | skills do hyperframes vêm do clone local, **mas o registry de ~426 blocos (`hyperframes add`, `catalog`) lê daqui** e responde "No items found" enquanto bloqueado. Liberar se o registry for usado; o clone em `/workspace/heygen-com/hyperframes/registry/` tem os arquivos |
...
| `cdn.jsdelivr.net` | **403** | **bloqueia o render do HyperFrames**: todo template carrega o GSAP (e outras libs do runtime) de lá. Liberar, ou vendorizar (ver gotcha) |
| `fonts.googleapis.com`, `fonts.gstatic.com` | **403** | Google Fonts não carrega em Remotion nem HyperFrames; fonte da marca entra como asset local |
| `hyperframes.heygen.com`, `api.heygen.com`, `api2.heygen.com` | **403** | docs, registry e render em nuvem do HyperFrames; render local não precisa |
| `unpkg.com`, `esm.sh`, `cdnjs.cloudflare.com` | **403** | outros CDNs que blocos do registry podem citar |
...
- **HyperFrames neste container (validado em 18/set/2026, render real de 10 s em 1080x1920).** Três coisas que o `hyperframes doctor` não resolve sozinho: (1) ele reprova `/usr/local/bin/ffmpeg` e não acha Chrome a menos que `HYPERFRAMES_FFMPEG_PATH`, `HYPERFRAMES_FFPROBE_PATH` e `HYPERFRAMES_BROWSER_PATH` estejam exportadas; o `headless_shell` do Playwright em `/opt/pw-browsers/chromium_headless_shell-1194/chrome-linux/headless_shell` serve, e `hyperframes browser ensure` tenta baixar de host bloqueado e trava. (2) `hyperframes init` confere skills em `raw.githubusercontent.com` (bloqueado); `HYPERFRAMES_SKIP_SKILLS=1` pula. (3) **O render é barrado com `sub_timeline_script_failure` porque o template carrega `https://cdn.jsdelivr.net/npm/gsap@3.14.2/dist/gsap.min.js`** e o CDN está fora da allowlist. Solução durável: liberar `cdn.jsdelivr.net` no environment. Contorno provado: `npm install --no-save gsap@3.14.2` no projeto, copiar `node_modules/gsap/dist/gsap.min.js` para `vendor/` e trocar o `src` no `index.html`. `scripts/hf.sh` cuida de (1) e (2); cadastrar as mesmas variáveis nas env vars do environment resolve para sessões novas sem o wrapper. whisper-cpp, Kokoro, MusicGen e Docker são opcionais (transcrição aqui é Scribe da ElevenLabs).
